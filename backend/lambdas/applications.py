"""
applications.py -- Lambda handling POST/GET for a user's job applications.

Table: Applications
  PK: userId (string)
  SK: canonicalId (string) -- canonical id of the job listing
  Attrs: jobTitle, company, domain, applyUrl, appliedAt, status

POST records that the user applied to a job (the real "Apply" button
records a row, not just opens a link). GET lists all applications for
the current user, most recent first.
"""
...
def _handle_get(user_id: str) -> dict:
    ...
    items.sort(key=lambda i: i.get("appliedAt", ""), reverse=True)
