...
Required IAM permissions (processResumeUploadRole -- create when deploying):
    ...
    dynamodb:GetItem, dynamodb:UpdateItem on Users (PoW score cache)
...
def _compute_pow_fingerprint(
    linkedin_url: str, github_handle, leetcode_handle, resume_bytes: bytes
) -> str:
    """Stable key for LinkedIn + optional handles + exact resume file bytes."""
    resume_digest = hashlib.sha256(resume_bytes).hexdigest()
    parts = [
        _normalize_linkedin_url(linkedin_url),
        _normalize_handle(github_handle),
        _normalize_handle(leetcode_handle),
        resume_digest,
    ]
    return hashlib.sha256("|".join(parts).encode("utf-8")).hexdigest()


def _lookup_cached_pow_score(user_id: str, fingerprint: str):
    try:
        response = _users_table.get_item(Key={"userId": user_id})
    except ClientError as e:
        error_code = e.response.get("Error", {}).get("Code", "Unknown")
        logger.warning("Users GetItem failed for PoW cache lookup: %s", error_code)
        return None

    item = response.get("Item") or {}
    if item.get("powScoreFingerprint") != fingerprint:
        return None

    try:
        pow_score = _clamp_pow_score(item.get("powScore"))
    except (TypeError, ValueError):
        return None

    breakdown = item.get("powBreakdown")
...
def _store_pow_cache(user_id: str, fingerprint: str, pow_score: int, breakdown: str):
    try:
        _users_table.update_item(
            Key={"userId": user_id},
            UpdateExpression=(
                "SET powScoreFingerprint = :fingerprint, "
                "powScore = :score, powBreakdown = :breakdown"
            ),
        ...
        error_code = e.response.get("Error", {}).get("Code", "Unknown")
        logger.warning("Users UpdateItem failed for PoW cache store: %s", error_code)
...
def _mark_awaiting_test(
    session_id: str, pow_score: int, breakdown: str, fingerprint: str | None = None
):
    update_expr = (
    ...
    if fingerprint:
        update_expr += ", powScoreFingerprint = :fingerprint"
        expr_values[":fingerprint"] = fingerprint

    _sessions_table.update_item(
        Key={"sessionId": session_id},
        UpdateExpression=update_expr + " REMOVE errorMessage",
        ExpressionAttributeNames={"#status": "status"},
...
def _process_record(record):
    ...
    try:
        ...
        cached = _lookup_cached_pow_score(user_id, fingerprint)
        if cached is not None:
            pow_score, breakdown = cached
            logger.info(
                "Reusing cached PoW score sessionId=%s userId=%s powScore=%s",
                session_id,
                user_id,
                pow_score,
            )
            ...
            _store_pow_cache(user_id, fingerprint, pow_score, breakdown)
            logger.info(
        ...
        _mark_awaiting_test(session_id, pow_score, breakdown, fingerprint)
        return {
            "key": key,
            "status": "awaiting_test",
            "powScore": pow_score,
