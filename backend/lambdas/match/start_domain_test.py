...
difficulty must be exactly one of easy, medium, hard, mixed.
...
TIME_LIMIT_HARD_MIN = 120
TIME_LIMIT_HARD_MAX = 150
...
def _time_limit_seconds(difficulty: str) -> int:
    ...
    return random.randint(TIME_LIMIT_HARD_MIN, TIME_LIMIT_HARD_MAX)
...
def _build_user_prompt(domain, skill, difficulty, in_demand_skills, extra=None):
    lines = [
        f"Domain: {domain}",
        f"Target skill: {skill}",
        f"Requested difficulty mix: {difficulty}",
...
def _generate_questions(domain, skill, difficulty, in_demand_skills):
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {
            "role": "user",
            "content": _build_user_prompt(domain, skill, difficulty, in_demand_skills),
        },
    ]
        ...
        messages = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {
                ...
                "content": _build_user_prompt(
                    domain, skill, difficulty, in_demand_skills, extra=RETRY_JSON_INSTRUCTION
                ),
            },
        ]
        content = _call_groq(messages)
...
def _attach_metadata(questions, difficulty_mode):
    decorated = []
    for index, question in enumerate(questions, start=1):
        if difficulty_mode == "mixed":
            difficulty = random.choice(QUESTION_DIFFICULTIES)
        else:
...
def lambda_handler(event, context):
    ...
    if difficulty not in VALID_DIFFICULTIES:
        return _response(
            ...
            {
                "error": (
                    "difficulty must be one of: " + ", ".join(VALID_DIFFICULTIES)
                )
            },
        )
    ...
    try:
        raw_questions = _generate_questions(domain, skill, difficulty, in_demand_skills)
    ...
    questions = _attach_metadata(raw_questions, difficulty)
